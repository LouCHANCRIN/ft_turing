...
from utils.error import NoSolutionError, NoTransitionError, InfiniteLoopError
        ...
        if current_state in known_states[tape]:
            if tape[head] in known_states[tape][current_state]:
                if head in known_states[tape][current_state][tape[head]]:
                    raise InfiniteLoopError(f"Infinite loop detected")
                else:
...
def perform_transition(
    ...
    if len(transition) != 1:
        raise NoTransitionError(f"Expected 1 transition for state {current_state} with read {tape[head]}, got {len(transition)}")
    if not silent:
...
def engine(machine: object, current_state: str, tape: list, head: int, count: int, silent: bool=False, known_states: object={}):
